...
describe('limited MVP results and settings', () => {
  ...
  it('shows local conditions, seasonal food, and stable guidance modules', async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ timezone: 'America/Los_Angeles', current: { temperature_2m: 72, apparent_temperature: 70, weather_code: 1 }, daily: { temperature_2m_max: [78], temperature_2m_min: [58], precipitation_probability_max: [20], sunrise: ['2026-07-16T05:40'], sunset: ['2026-07-16T20:55'] } }) })
    ...
    expect(frame).toHaveClass('daylight-theme')
    ...
    await waitFor(() => expect(frame).toHaveAttribute('data-daylight-source', 'solar'))
    expect(fetchMock).toHaveBeenCalledTimes(1)
    expect(frame?.dataset.daylightPhase).toMatch(/^(midday|sunset|twilight|night)$/)
    expect(frame?.style.getPropertyValue('--ambient-x')).toMatch(/^\d+(?:\.\d+)?%$/)
  ...
  it('shows one contextual location invitation and makes no weather request without location', async () => {
    ...
    vi.stubGlobal('fetch', fetchMock)
    const user = userEvent.setup()
    const { container } = renderAt('/today', { resultsReached: true, profile: coreProfileWithoutLocation('Alex') })
    expect(container.querySelector('.app-frame')).toHaveAttribute('data-daylight-source', 'browser')
    expect(screen.getByRole('button', { name: 'Mark recommendation complete' })).toBeInTheDocument()
