  ...
  return (
    <AppShell>
      <Routes>
        ...
        <Route path="/profile/location" element={<Guard allowed={state.accountCreated && Boolean(state.profile.preferredName)} redirect={resumePath}><Suspense fallback={<section className="screen"><p role="status">Loading location options…</p></section>}><LocationProfileScreen /></Suspense></Guard>} />
        <Route path="/profile/food" element={<Guard allowed={state.accountCreated && Boolean(state.profile.location)} redirect={resumePath}><FoodProfileScreen /></Guard>} />
